...
{
	auto hr = DirectInput8Create(GetModuleHandle(0), DIRECTINPUT_VERSION, IID_IDirectInput8, (void**)(&input), nullptr);
	if (FAILED(hr))
	{
...
{
	auto hr = dev->SetCooperativeLevel(win.lock()->Get(), DISCL_NONEXCLUSIVE | DISCL_BACKGROUND);
	if (FAILED(hr))
	{
		OutputDebugString(_T("\nキーの協調レベルのセット：失敗\n"));
		return hr;
